...
def generate_base_flights(date_obj, n_flights):
    ...
    for _ in range(n_flights):
        ...
        departure = round_to_5_min(datetime(date_obj.year, date_obj.month, date_obj.day,
...
def clear_old_flights(conn, start_date):
    """Delete flights before today"""
    with conn.cursor() as cur:
        cur.execute("DELETE FROM daily_flights WHERE departure_time::date < %s", (start_date,))
    conn.commit()

def update_schedule(num_days_ahead=365, num_flights_per_day=1100):
    """Maintain 365-day schedule: remove past, add missing future days"""
    ...
    try:
        ...
        for day_offset in range(days_to_insert):
            ...
            flights = generate_base_flights(schedule_date, num_flights_per_day)
            records = [
                (
                    f["flight_no"], f["departure_time"], f["arrival_time"],
                    f["origin_iata"], f["destination_iata"], f["aircraft"], f["base_cost_cad"]
                )
                for f in flights
            ]
            with conn.cursor() as cur:
                execute_batch(cur, """
                    INSERT INTO daily_flights (
                        ...
                        origin_iata, destination_iata, aircraft, base_cost_cad
